...
class PublicProfileGuestBookWidget extends StatefulWidget {
  final UserModel userModel;

  const PublicProfileGuestBookWidget({Key? key, required this.userModel})
      : super(key: key);
...
class GuestBookWidgetView extends WidgetView<PublicProfileGuestBookWidget,
    GuestBookWidgetController> {
  const GuestBookWidgetView(GuestBookWidgetController state, {super.key})
      : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<GuestbookCubit, GuestbookState>(
      bloc: state.guestbookCubit,
      listener: (context, guestbookStatus) {
        if (guestbookStatus.status == GuestBookStatus.deleteGuestBookSuccess) {
          state.refreshGuestBook();
          // state.pagingController.itemList?.removeWhere(
          //     (element) => element.id == state.pagingController.);
          // state.pagingController.refresh();
        }
        if (guestbookStatus.status == GuestBookStatus.createGuestBookSuccess) {
          state.refreshGuestBook();
        }
      },
      child: Column(
              ...
              children: [
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        "Sign @${widget.userModel.displayName}’s Guestbook",
                ...
                CustomButtonWidget(
                  ...
                  onPressed: () {
                    context.push(
                        context.generateRoutePath(subLocation: createGuestBook),
                        extra: {
...
class GuestBookWidgetController extends State<PublicProfileGuestBookWidget> {
  // page key is page index. Starting from 0, 1, 2 .........
  final PagingController<int, GuestBookModel> pagingController =
      PagingController(
  ...
  void initState() {
    ...
    pagingController.addPageRequestListener((pageKey) async {
      final response = await fetchGuestBookFeeds(
          pageKey: pageKey, userName: widget.userModel.username!);
      if (response.isLeft()) {
  ...
  Future<dartz.Either<String, List<GuestBookModel>>> fetchGuestBookFeeds(
      {required int pageKey, required String userName}) async {
    return await guestbookCubit.fetchGuestBook(
        userName: userName, pageKey: pageKey);
  }

  @override
  Widget build(BuildContext context) => GuestBookWidgetView(this);

  void refreshGuestBook() async{
    final response = await fetchGuestBookFeeds(
        pageKey: 0, userName: widget.userModel.username!);
    if (response.isLeft()) {
      return;
    }
    final guestBook = response.asRight();
    pagingController.value =
        PagingState(nextPageKey: 1, itemList: guestBook);
  }
}
