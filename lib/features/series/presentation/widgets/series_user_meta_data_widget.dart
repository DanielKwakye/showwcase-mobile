...
class SeriesUserMetaDataWidget extends StatefulWidget {
  ...
  const SeriesUserMetaDataWidget({required this.user,
    this.showUsername = false,
    this.withTime,
    this.showMoreMenu = true,
    Key? key, required this.series}) : super(key: key);

  @override
  State<SeriesUserMetaDataWidget> createState() => _SeriesUserMetaDataWidgetState();
}

class _SeriesUserMetaDataWidgetState extends State<SeriesUserMetaDataWidget> {
              ...
              if(widget.showMoreMenu) ... {
                PopupMenuButton<String>(
                  ...
                  itemBuilder: (ctx) => [
                    ...
                    _buildPopupMenuItem(context, 'Report', Icons.flag_outlined, "report"),
                  ],
                  child: Container(
                    height: 24,
                    width: 24,
  ...
  void _onMoreMenuItemTapped(String menu){
     if (menu == "copy"){
      final copyUrl = "${ApiConfig.websiteUrl}/series/${widget.series.id}/${widget.series.slug}";
  ...
  void _submitReport(BuildContext ctx) {
    seriesCubit.reportSeries(message: reportReason, seriesId: widget.series.id!);
    Navigator.of(context).pop();
  }
