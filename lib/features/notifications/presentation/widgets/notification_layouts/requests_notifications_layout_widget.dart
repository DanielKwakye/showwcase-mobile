...
class RequestsNotificationsLayoutWidget extends StatefulWidget {

  final NotificationCategory category;
  final List<String> type;
  final ScrollController scrollController;
  const RequestsNotificationsLayoutWidget({
  ...
  @override
  State<RequestsNotificationsLayoutWidget> createState() => _NotificationsLayoutWidgetController();
}
...
class _NotificationsLayoutWidgetView extends WidgetView<RequestsNotificationsLayoutWidget, _NotificationsLayoutWidgetController> {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSharedRefreshIndicator(
      onRefresh: () async{
        final response = await state.fetchRequestNotifications(0);
        if(response.isLeft()){
          return;
        }
        ...
        builderDelegate: PagedChildBuilderDelegate<NotificationModel>(
          itemBuilder: (context, item, index) {
            ...
            return Container(
              ...
              child: BlocSelector<NotificationRequestsCubit, NotificationState, NotificationModel>(
                selector: (state) {
...
class _NotificationsLayoutWidgetController extends State<RequestsNotificationsLayoutWidget> with AutomaticKeepAliveClientMixin<RequestsNotificationsLayoutWidget> {
  late NotificationRequestsCubit _notificationCubit;
  ...
  initState(){
    super.initState();
    _notificationCubit = context.read<NotificationRequestsCubit>();
    pagingController.addPageRequestListener((pageKey) async {
      final response = await fetchRequestNotifications(pageKey);
      if(response.isLeft()){
    ...
    OneSignal.Notifications.addForegroundWillDisplayListener((event) async {
      final response = await _notificationCubit.fetchRequestNotifications(0);
      if(response.isLeft()){
        return;
      }
      final notifications = response.asRight();
  ...
  Future<dartz.Either<String, List<NotificationModel>>> fetchRequestNotifications(int pageKey) async {
    return await _notificationCubit.fetchRequestNotifications(pageKey);
  }
