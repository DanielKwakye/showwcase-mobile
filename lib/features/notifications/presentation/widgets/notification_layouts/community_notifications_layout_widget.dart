...
class CommunityNotificationsLayoutWidget extends StatefulWidget {

  final NotificationCategory category;
  final List<String> type;
  final ScrollController scrollController;
  const CommunityNotificationsLayoutWidget({
  ...
  @override
  NotificationsLayoutWidgetController createState() => NotificationsLayoutWidgetController();
}
...
class _NotificationsLayoutWidgetView extends WidgetView<CommunityNotificationsLayoutWidget, NotificationsLayoutWidgetController> {

  const _NotificationsLayoutWidgetView(NotificationsLayoutWidgetController state) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSharedRefreshIndicator(
      onRefresh: () async{
        final response = await state.fetchCommunityNotifications(0);
        if(response.isLeft()){
          return;
        }
        ...
        builderDelegate: PagedChildBuilderDelegate<NotificationModel>(
          itemBuilder: (context, item, index) {
            ...
            return Container(
              ...
              child: BlocSelector<NotificationCommunityCubit, NotificationState, NotificationModel>(
                selector: (state) {
...
class NotificationsLayoutWidgetController extends State<CommunityNotificationsLayoutWidget> with AutomaticKeepAliveClientMixin<CommunityNotificationsLayoutWidget> {

  late NotificationCommunityCubit _notificationCubit;
  ...
  initState(){
    super.initState();
    _notificationCubit = context.read<NotificationCommunityCubit>();
    pagingController.addPageRequestListener((pageKey) async {
      final response = await fetchCommunityNotifications(pageKey);
      if(response.isLeft()){
    ...
    OneSignal.Notifications.addForegroundWillDisplayListener((event) async {
      final response = await _notificationCubit.fetchCommunityNotifications(0);
      if(response.isLeft()){
        return;
      }
      final notifications = response.asRight();
  ...
  Future<dartz.Either<String, List<NotificationModel>>> fetchCommunityNotifications(int pageKey) async {
    return await _notificationCubit.fetchCommunityNotifications(pageKey);
  }
