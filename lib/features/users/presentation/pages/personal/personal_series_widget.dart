...
class PersonalProfileSeriesWidget extends StatefulWidget {

  const PersonalProfileSeriesWidget({Key? key}) : super(key: key);

  @override
  PersonalSeriesWidgetController createState() => PersonalSeriesWidgetController();
}
...
class PersonalSeriesWidgetView extends WidgetView<PersonalProfileSeriesWidget, PersonalSeriesWidgetController> {

  const PersonalSeriesWidgetView(PersonalSeriesWidgetController state, {super.key}) : super(state);
            ...
            return Container(
                ...
                child: BlocSelector<ProfileSeriesFeedsCubit, SeriesState, SeriesModel>(
                  ...
                  builder: (context, updatedSeriesItem) {
                    return SeriesItemWidget(seriesItem: updatedSeriesItem, itemIndex: index, key: ValueKey(item.id),pageName: personalProfilePage,);
                  },
                )
            );
          },
...
class PersonalSeriesWidgetController extends State<PersonalProfileSeriesWidget> with AutomaticKeepAliveClientMixin {
  ...
  Future<dartz.Either<String, List<SeriesModel>>> fetchProfileSeriesFeeds(int pageKey) async {
    ...
    return await seriesFeedsCubit.fetchProfileSeriesFeeds(pageKey, currentUser.username!);
  }

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return PersonalSeriesWidgetView(this);
  }
