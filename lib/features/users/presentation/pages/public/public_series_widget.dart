...
class PublicProfileSeriesWidget extends StatefulWidget {
  final UserModel userModel ;

  const PublicProfileSeriesWidget({Key? key, required this.userModel}) : super(key: key);

  @override
  PublicSeriesWidgetController createState() => PublicSeriesWidgetController();
}
...
class PublicSeriesWidgetView extends WidgetView<PublicProfileSeriesWidget, PublicSeriesWidgetController> {

  const PublicSeriesWidgetView(PublicSeriesWidgetController state, {super.key}) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSharedRefreshIndicator(
      onRefresh: () async {
        final response = await state.fetchProfileSeriesFeeds(0);
        if(response.isLeft()){
          return;
        }
        ...
        builderDelegate: PagedChildBuilderDelegate<SeriesModel>(
          itemBuilder: (context, item, index) {
            return Container(
                ...
                child: BlocSelector<ProfileSeriesFeedsCubit, SeriesState, SeriesModel>(
                  selector: (seriesState) {
                    return seriesState.series.firstWhere((element) => element.id == item.id);
                  },
                  builder: (context, updatedSeriesItem) {
                    return SeriesItemWidget(seriesItem: updatedSeriesItem, itemIndex: index, key: ValueKey(item.id),pageName: publicProfilePage,);
                  },
                )
            );
          },
...
class PublicSeriesWidgetController extends State<PublicProfileSeriesWidget> with AutomaticKeepAliveClientMixin {
  ...
  late ProfileSeriesFeedsCubit seriesFeedsCubit;


  @override
  void initState() {
    seriesFeedsCubit = context.read<ProfileSeriesFeedsCubit>();
    pagingController.addPageRequestListener((pageKey) async {
      final response = await fetchProfileSeriesFeeds(pageKey);
      if(response.isLeft()){
  ...
  Future<dartz.Either<String, List<SeriesModel>>> fetchProfileSeriesFeeds(int pageKey) async {
    return await seriesFeedsCubit.fetchProfileSeriesFeeds(pageKey,widget.userModel.username!);
  }

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return PublicSeriesWidgetView(this);
  }
