...
class PersonalSocialWidget extends StatelessWidget with LaunchExternalAppMixin {
  const PersonalSocialWidget({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocSelector<UserProfileCubit, UserProfileState, UserSocialModel?>(
      selector: (userState) {
        ...
        if(!userState.social.keys.contains(currentUser!.username!)) {
          return null;
        }

        final userInfo = userState.social[currentUser.username];
        return userInfo;
      },
