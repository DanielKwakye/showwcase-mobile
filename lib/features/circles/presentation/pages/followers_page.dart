...
class FollowersPage extends StatefulWidget {
  ...
  const FollowersPage({Key? key, required this.userModel}) : super(key: key);

  @override
  FollowersPageController createState() => FollowersPageController();
}
...
class _FollowersPageView extends WidgetView<FollowersPage, FollowersPageController> {

  const _FollowersPageView(FollowersPageController state) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSharedRefreshIndicator(
      onRefresh: () async {
        final response = await state.fetchFollowers(0);
        if(response.isLeft()){
          return;
        }
          ...
          itemBuilder: (context, item, index) {
            ...
            return BlocSelector<UserProfileCubit, UserProfileState, UserModel>(
              selector: (profileState) {
                return profileState.followers[widget.userModel.username!]!.firstWhere((element) => element.id == item.id);
              },
...
class FollowersPageController extends State<FollowersPage> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return _FollowersPageView(this);
  }
  ...
  void initState() {
    ...
    pagingController.addPageRequestListener((pageKey) async {
      ...
      final response = await fetchFollowers(pageKey);
      if(response.isLeft()){
  ...
  Future<dartz.Either<String, List<UserModel>>> fetchFollowers(int pageKey) async {
    return await userProfileCubit.fetchUserFollowers(user: widget.userModel, pageKey: pageKey);
  }
