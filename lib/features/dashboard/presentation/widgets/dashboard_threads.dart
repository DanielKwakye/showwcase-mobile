...
import 'package:showwcase_v3/features/dashboard/presentation/widgets/dashboard_thread_item.dart';
...
class DashboardThreadPage extends StatefulWidget {

  const DashboardThreadPage({Key? key}) : super(key: key);

  @override
  DashboardThreadPageController createState() => DashboardThreadPageController();
}
...
class _DashboardThreadPageView extends WidgetView<DashboardThreadPage, DashboardThreadPageController> {

  const _DashboardThreadPageView(DashboardThreadPageController state) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSharedRefreshIndicator(
      ...
      child: BlocListener<DashboardThreadsCubit, ThreadState>(
        listenWhen: (_, next){
          ...
          builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
            itemBuilder: (context, item, index) =>
                Container(
                    ...
                    child: BlocSelector<DashboardThreadsCubit, ThreadState, ThreadModel?>(
                      selector: (state) {return state.threads.firstWhereOrNull((element) => element.id == item.id);},
                      builder: (context, thread) {
                        return DashboardThreadItemWidget(threadModel: thread ?? item,);
                      },
                    )
                ),
...
class DashboardThreadPageController extends State<DashboardThreadPage> with AutomaticKeepAliveClientMixin{
  ...
  late DashboardThreadsCubit threadCubit;

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return _DashboardThreadPageView(this);
  }

  @override
  void initState() {
    super.initState();
    threadCubit = context.read<DashboardThreadsCubit>();
