...
class SeeMoreProposedCommunities extends StatefulWidget {
  const SeeMoreProposedCommunities({Key? key}) : super(key: key);

  @override
  State<SeeMoreProposedCommunities> createState() => _SeeMoreProposedCommunitiesState();
}

class _SeeMoreProposedCommunitiesState extends State<SeeMoreProposedCommunities> {
  ...
  Future<dartz.Either<String, List<CommunityModel>>> fetchCommunities(int pageKey) async {
    return await _communitiesListCubit.fetchProposedCommunities(pageKey: pageKey);
  }
      ...
      body: CustomScrollView(
        slivers: [
          const CustomInnerPageSliverAppBar(
            pageTitle: 'Newly Proposed Communities',
            pinned: true,
          ),
          PagedSliverList.separated(
            ...
            builderDelegate: PagedChildBuilderDelegate<CommunityModel>(
              itemBuilder: (context, item, index) {
                ...
                return BlocSelector<ProposedCommunitiesCubit, CommunityState, CommunityModel>(
                  selector: (state) {
                    return state.communities.firstWhere((element) =>
                    element.id == item.id);
                  },
