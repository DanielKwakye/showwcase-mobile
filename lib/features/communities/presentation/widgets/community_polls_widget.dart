...
class CommunitiesPollsThread extends StatefulWidget {
  final CommunityModel communityModel;

  const CommunitiesPollsThread(
      {Key? key, required this.communityModel, })
      : super(key: key);

  @override
  CommunitiesPollsThreadController createState() =>
      CommunitiesPollsThreadController();
}
...
class CommunitiesPollsThreadView
    extends WidgetView<CommunitiesPollsThread, CommunitiesPollsThreadController> {
  const CommunitiesPollsThreadView(CommunitiesPollsThreadController state,
      {super.key})
      : super(state);
            ...
            builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
              itemBuilder: (context, item, index) =>
                  Container(
                      ...
                      child: BlocSelector<CommunityPollThreadsCubit, ThreadState, ThreadModel?>(
                        selector: (state) {return state.threads.firstWhereOrNull((element) => element.id == item.id);},
                        builder: (context, thread) {
                          return ThreadItemWidget(threadModel: thread ?? item,pageName: 'community_polls_feeds',);
                        },
                      )
                  ),
...
class CommunitiesPollsThreadController extends State<CommunitiesPollsThread>
  ...
  late CommunityPollThreadsCubit threadCubit;
  ...
  void initState() {
    ...
    threadCubit = context.read<CommunityPollThreadsCubit>();
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return CommunitiesPollsThreadView(this);
  }

  Future<dartz.Either<String, List<ThreadModel>>> fetchLinkThreadFeeds({required int pageKey,required String? tag}) async {
    return await threadCubit.fetchLinkThreadFeeds(pageKey:pageKey, communityName: widget.communityModel.slug ?? '',tag: tag,feedType: 'poll');
  }
