...
class CommunitiesMediaThread extends StatefulWidget {
  final CommunityModel communityModel;

  const CommunitiesMediaThread(
      {Key? key, required this.communityModel, })
      : super(key: key);

  @override
  CommunitiesMediaThreadController createState() =>
      CommunitiesMediaThreadController();
}
...
class CommunitiesMediaThreadView
    extends WidgetView<CommunitiesMediaThread, CommunitiesMediaThreadController> {
  const CommunitiesMediaThreadView(CommunitiesMediaThreadController state,
      {super.key})
      : super(state);
            ...
            builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
              itemBuilder: (context, item, index) =>
                  Container(
                      ...
                      child: BlocSelector<CommunityMediaThreadsCubit, ThreadState, ThreadModel?>(
                        selector: (state) {return state.threads.firstWhereOrNull((element) => element.id == item.id);},
                        builder: (context, thread) {
                          return ThreadItemWidget(threadModel: thread ?? item,pageName: 'community_media_feeds',);
                        },
                      )
                  ),
...
class CommunitiesMediaThreadController extends State<CommunitiesMediaThread>
  ...
  late CommunityMediaThreadsCubit mediaThreadCubit;
  ...
  void initState() {
    ...
    mediaThreadCubit = context.read<CommunityMediaThreadsCubit>();
    pagingController.addPageRequestListener((pageKey) async {
      final response = await fetchMediaThreadFeeds(pageKey: pageKey,tag: selectedTag);
      if(response.isLeft()){
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return CommunitiesMediaThreadView(this);
  }

  Future<dartz.Either<String, List<ThreadModel>>> fetchMediaThreadFeeds({required int pageKey,required String? tag}) async {
    return await mediaThreadCubit.fetchMediaThreadFeeds(pageKey:pageKey, communityName: widget.communityModel.slug ?? '',tag: tag,feedType: 'media');
  }
