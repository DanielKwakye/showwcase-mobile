...
class CommunitiesLinkThread extends StatefulWidget {
  final CommunityModel communityModel;

  const CommunitiesLinkThread(
      {Key? key, required this.communityModel, })
      : super(key: key);

  @override
  CommunitiesLinkThreadController createState() =>
      CommunitiesLinkThreadController();
}
...
class CommunitiesLinkThreadView
    extends WidgetView<CommunitiesLinkThread, CommunitiesLinkThreadController> {
  const CommunitiesLinkThreadView(CommunitiesLinkThreadController state,
      {super.key})
      : super(state);
            ...
            builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
              itemBuilder: (context, item, index) =>
                  Container(
                      ...
                      child: BlocSelector<CommunityLinkThreadsCubit, ThreadState, ThreadModel?>(
                        selector: (state) {return state.threads.firstWhereOrNull((element) => element.id == item.id);},
                        builder: (context, thread) {
                          return ThreadItemWidget(threadModel: thread ?? item,pageName: 'community_links_feeds',);
                        },
                      )
                  ),
...
class CommunitiesLinkThreadController extends State<CommunitiesLinkThread>
  ...
  late CommunityLinkThreadsCubit linksThreadCubit;
  ...
  void initState() {
    ...
    linksThreadCubit = context.read<CommunityLinkThreadsCubit>();
    pagingController.addPageRequestListener((pageKey) async {
      final response = await fetchLinkThreadFeeds( pageKey: pageKey,tag: selectedTag);
      if(response.isLeft()){
  ...
  Widget build(BuildContext context) {
    super.build(context);
    return CommunitiesLinkThreadView(this);
  }

  Future<dartz.Either<String, List<ThreadModel>>> fetchLinkThreadFeeds({required int pageKey,required String? tag}) async {
    return await linksThreadCubit.fetchLinkThreadFeeds(pageKey:pageKey, communityName: widget.communityModel.slug ?? '',tag: tag,feedType: 'links');
  }
