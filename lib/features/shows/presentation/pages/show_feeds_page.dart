...
class ShowFeedsPage extends StatefulWidget {

  const ShowFeedsPage({Key? key}) : super(key: key);

  @override
  ShowFeedsPageController createState() => ShowFeedsPageController();
}
...
class _ShowFeedsPageView extends WidgetView<ShowFeedsPage, ShowFeedsPageController> {

  const _ShowFeedsPageView(ShowFeedsPageController state) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return CustomSharedRefreshIndicator(
      onRefresh: () async {
        ...
        state.pagingController.value = PagingState(nextPageKey: 1, itemList: shows);
      },
        ...
        builderDelegate: PagedChildBuilderDelegate<ShowModel>(
          itemBuilder: (context, item, index) {
            return Container(
              ...
              child: BlocSelector<ShowFeedsCubit, ShowsState, ShowModel>(
                selector: (state) {
                  return state.shows.firstWhere((element) => element.id == item.id);
                },
                builder: (context, show) {
                  return ShowItemWidget(showModel: show,pageName: shows,);
                },
              )
              ,
            );
...
class ShowFeedsPageController extends State<ShowFeedsPage> with AutomaticKeepAliveClientMixin{
  ...
  late ShowFeedsCubit showsFeedsCubit;
  late StreamSubscription<ShowsState> showStateStreamSubscription;

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return _ShowFeedsPageView(this);
  }

  @override
  void initState() {
    super.initState();
    showsFeedsCubit = context.read<ShowFeedsCubit>();
    showsFeedsCubit.fetchShowCategories();
    ...
    showStateStreamSubscription = showsFeedsCubit.stream.listen((event) {
      if(event.status == ShowsStatus.selectShowFeedCategoryCompleted){
  ...
  Future<dartz.Either<String, List<ShowModel>>> fetchShows(int pageKey) async {
    return await showsFeedsCubit.fetchShowFeeds(pageKey);
  }


  @override
  void dispose() {
    showStateStreamSubscription.cancel();
    super.dispose();
  }
