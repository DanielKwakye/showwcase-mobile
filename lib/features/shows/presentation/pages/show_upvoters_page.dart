...
class ShowUpvotersPage extends StatefulWidget {

  final ShowModel showModel;
  const ShowUpvotersPage({Key? key, required this.showModel}) : super(key: key);

  @override
  ShowUpvotersPageController createState() => ShowUpvotersPageController();
}
...
class _ShowUpvotersPageView extends WidgetView<ShowUpvotersPage, ShowUpvotersPageController> {

  const _ShowUpvotersPageView(ShowUpvotersPageController state) : super(state);
        ...
        body: NestedScrollView(headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
          return [
            SliverAppBar(
              ...
              elevation: 0,
              title: Text("Liked by", style: TextStyle(color: theme.colorScheme.onBackground, fontWeight: FontWeight.w700,fontSize: defaultFontSize ),),
            ...
            builderDelegate: PagedChildBuilderDelegate<UserModel>(
              itemBuilder: (context, item, index) {
                return Padding(
                  ...
                  child: UserListTileWidget(userModel: item),
                );
              },
...
class ShowUpvotersPageController extends State<ShowUpvotersPage> {
  ...
  late ShowUpVotersCubit cubit;

  @override
  Widget build(BuildContext context) => _ShowUpvotersPageView(this);

  @override
  void initState() {
    super.initState();
    cubit = context.read<ShowUpVotersCubit>();
  ...
  Future<dartz.Either<String, List<UserModel>>> fetchVoters(int pageKey) async {
    return await cubit.fetchShowUpVoters(pageKey: pageKey, show: widget.showModel);
  }
