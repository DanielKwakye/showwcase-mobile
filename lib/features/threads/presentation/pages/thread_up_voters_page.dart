...
class ThreadUpVotersPage extends StatefulWidget {

  final ThreadModel thread;
  const ThreadUpVotersPage({Key? key, required this.thread}) : super(key: key);

  @override
  ThreadUpVotersPageController createState() => ThreadUpVotersPageController();
}
...
class _ThreadVotersPageView extends WidgetView<ThreadUpVotersPage, ThreadUpVotersPageController> {

  const _ThreadVotersPageView(ThreadUpVotersPageController state) : super(state);
        ...
        body: NestedScrollView(headerSliverBuilder: (BuildContext context, bool innerBoxIsScrolled) {
          return [
            SliverAppBar(
              ...
              elevation: 0,
              title: Text("Upvoted by", style: TextStyle(color: theme.colorScheme.onBackground, fontWeight: FontWeight.w700,fontSize: defaultFontSize ),),
...
class ThreadUpVotersPageController extends State<ThreadUpVotersPage> {
  ...
  late ThreadUpVotersCubit cubit;
  ...
  void initState() {
    super.initState();
    cubit = context.read<ThreadUpVotersCubit>();
  ...
  Future<dartz.Either<String, List<UserModel>>> fetchVoters(int pageKey) async {
    return await cubit.fetchThreadUpVoters(pageKey: pageKey, thread: widget.thread);
  }
