...
class LatestThreadFeedsPage extends StatefulWidget {

  const LatestThreadFeedsPage({Key? key}) : super(key: key);

  @override
  LatestThreadFeedsPageController createState() => LatestThreadFeedsPageController();
}
...
class _LatestThreadFeedsPageView extends WidgetView<LatestThreadFeedsPage, LatestThreadFeedsPageController> {

  const _LatestThreadFeedsPageView(LatestThreadFeedsPageController state) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<LatestFeedsThreadCubit, ThreadState>(
      listenWhen: (_, next){
          ...
          builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
            itemBuilder: (context, item, index) =>
                Container(
                    ...
                    child: BlocSelector<LatestFeedsThreadCubit, ThreadState, ThreadModel?>(
                      selector: (state) {
                        return state.threads.firstWhereOrNull((element) => element.id == item.id);
                      },
                      builder: (context, thread) {
                        return ThreadItemWidget(threadModel: thread ?? item,pageName: 'latest_thread_feeds');
                      },
                    )
                  // ThreadItemWidget<LatestFeedsThreadCubit>(threadModel: item,)
                ),
...
class LatestThreadFeedsPageController extends State<LatestThreadFeedsPage> with AutomaticKeepAliveClientMixin {
  ...
  late LatestFeedsThreadCubit threadCubit;

  @override
  Widget build(BuildContext context) {
    super.build(context);
    return _LatestThreadFeedsPageView(this);
  }

  @override
  void initState() {
    super.initState();
    threadCubit = context.read<LatestFeedsThreadCubit>();
