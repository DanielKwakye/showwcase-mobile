...
class NewsThreadFeedsPage extends StatefulWidget {


  const NewsThreadFeedsPage({Key? key}) : super(key: key);

  @override
  NewsThreadFeedsPageController createState() => NewsThreadFeedsPageController();
}
...
class _NewsThreadFeedsPageView extends WidgetView<NewsThreadFeedsPage, NewsThreadFeedsPageController> {

  const _NewsThreadFeedsPageView(NewsThreadFeedsPageController state) : super(state);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<NewsFeedsThreadCubit, ThreadState>(
      listenWhen: (_, next){
          ...
          builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
            itemBuilder: (context, item, index) =>
                Container(
                    ...
                    child: BlocSelector<NewsFeedsThreadCubit, ThreadState, ThreadModel?>(
                      selector: (state) {
                        return state.threads.firstWhereOrNull((element) => element.id == item.id);
                      },
                      builder: (context, thread) {
                        return ThreadItemWidget(threadModel: thread ?? item,pageName: 'news_thread_feeds',);
                      },
                    )
                ),
...
class NewsThreadFeedsPageController extends State<NewsThreadFeedsPage> with AutomaticKeepAliveClientMixin {
  ...
  late NewsFeedsThreadCubit threadCubit;


  @override
  Widget build(BuildContext context) {
    super.build(context);
    return _NewsThreadFeedsPageView(this);
  }

  @override
  void initState() {
    super.initState();
    threadCubit = context.read<NewsFeedsThreadCubit>();
