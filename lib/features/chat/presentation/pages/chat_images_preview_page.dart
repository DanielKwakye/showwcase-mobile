...
class ChatImagesPreviewPage extends StatefulWidget {

  final List<String> galleryItems;
  final int initialPageIndex;
  final ChatMessageModel chat;
  const ChatImagesPreviewPage({Key? key, required this.galleryItems,
    this.initialPageIndex = 0, required this.chat}) : super(key: key);

  @override
  ChatImagesPreviewPageController createState() => ChatImagesPreviewPageController();
}
...
class _ChatImagesPreviewPageView extends WidgetView<ChatImagesPreviewPage, ChatImagesPreviewPageController> {

  const _ChatImagesPreviewPageView(ChatImagesPreviewPageController state) : super(state);
            ...
            return Scaffold(
                        ...
                        child: Row(
                          ...
                          children: [
                            CustomUserAvatarWidget(
                              networkImage: widget.chat.user?.profilePictureKey, username: widget.chat.user?.username,
                              borderSize: 0,
                              borderColor: kAppBlack,
                              size: 30,
                            ),
                            const SizedBox(width: 10,),
                            Text(widget.chat.user?.displayName ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.bodyMedium?.copyWith(color: kAppWhite),)
                ...
                body: Stack(
                  children: [
                    ...
                    /// Show data  -----------------
                    if( !widget.chat.text.isNullOrEmpty() ) ... {
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(widget.chat.text!, style: theme.textTheme.bodyMedium?.copyWith(fontSize: defaultFontSize, color: kAppWhite),),
...
class ChatImagesPreviewPageController extends State<ChatImagesPreviewPage> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) => _ChatImagesPreviewPageView(this);
