...
class BookmarkShowFeedsPage extends StatefulWidget {

  const BookmarkShowFeedsPage({Key? key}) : super(key: key);
...
class _BookmarkShowFeedsPageView extends WidgetView<BookmarkShowFeedsPage, BookmarkShowFeedsPageController> {
        ...
        builderDelegate: PagedChildBuilderDelegate<ShowModel>(
          itemBuilder: (context, item, index) {
            return Container(
              ...
              child: BlocSelector<BookmarkShowsCubit, ShowsState, ShowModel>(
                selector: (state) {

                  return state.shows.firstWhere((element) => element.id == item.id);
                },
                builder: (context, show) {
                  return ShowItemWidget(showModel: show,pageName: 'bookmark_shows_feeds',);
                },
              )
              ,
            );
...
class BookmarkShowFeedsPageController extends State<BookmarkShowFeedsPage> {
  ...
  late BookmarkShowsCubit bookmarkShowsCubit;
  ...
  void initState() {
    super.initState();
    bookmarkShowsCubit = context.read<BookmarkShowsCubit>();
  ...
  Future<dartz.Either<String, List<ShowModel>>> fetchShows(int pageKey) async {
    return await bookmarkShowsCubit.fetchBookmarkedShows(pageKey);
  }
