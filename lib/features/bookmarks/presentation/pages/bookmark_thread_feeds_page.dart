...
class BookmarkThreadFeedsPage extends StatefulWidget {

  const BookmarkThreadFeedsPage({Key? key}) : super(key: key);
...
class _BookmarkThreadFeedsPageView extends WidgetView<BookmarkThreadFeedsPage, BookmarkThreadFeedsPageController> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocListener<BookmarkThreadsCubit, ThreadState>(
      listenWhen: (_, next){
          ...
          builderDelegate: PagedChildBuilderDelegate<ThreadModel>(
            itemBuilder: (context, item, index) =>
                Container(
                    ...
                    child: BlocSelector<BookmarkThreadsCubit, ThreadState, ThreadModel?>(
                      selector: (state) {return state.threads.firstWhereOrNull((element) => element.id == item.id);},
                      builder: (context, thread) {
                        return ThreadItemWidget(threadModel: thread ?? item,pageName: 'bookmark_thread_feeds ',);
                      },
                    )
                ),
...
class BookmarkThreadFeedsPageController extends State<BookmarkThreadFeedsPage> {
  ...
  late BookmarkThreadsCubit threadCubit;
  ...
  void initState() {
    super.initState();
    threadCubit = context.read<BookmarkThreadsCubit>();
