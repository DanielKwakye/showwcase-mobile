...
class JobTypesPage extends StatefulWidget {
  ...
  @override
  State<JobTypesPage> createState() => _JobTypesPageState();
}
            ...
            builder: (context, jobsState) {
              ...
              if(jobsState.status == JobStatus.jobsFiltersFetchedSuccessful && jobsState.jobFilters != null){
                return  SliverList(delegate: SliverChildListDelegate(
                    [
                      ...(jobsState.jobFilters?.types ?? <Map<String, dynamic>>[]).map((skill) =>
                          BlocSelector<JobsCubit, JobsState, Map<String, dynamic>?>(
                            selector: (jobState) {
                              final mapIndex =  jobState.jobFilters!.types!.indexWhere((element) => element["filter"] == skill["filter"]);
                              if(mapIndex >= 0){
                                final map = jobState.jobFilters!.types![mapIndex];
                                return map;
                              }
                              return null;
                            },
                            builder: (context, filterItem) {
                              ...
                              return CheckboxListTile(
                                  title: Text(
                                    (filterItem["filter"] as JobTypeModel).label ?? '',
                                    style: TextStyle(
